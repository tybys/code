exports.list = function(req, res) {
	req.getConnection(function (err, connection) {
		...
		/*almost good*/connection.query('SELECT * FROM driver LEFT JOIN transport ON transport.driver_binding = driver.d_id', function (err, results, fields) {
			if (err) throw err;
			else {
...
exports.add = function (req, res) {
	...
	//var data = {name: input.name,surname: input.surname,patronymic: input.patronymic,birthdate: input.birthdate,open_category: input.open_category,license: input.license,photo: input.photo,phone_number: input.phone_number,comment: input.comment,status: input.status,bus_binding: input.bus_binding,company_id: input.company_id};

	req.getConnection(function (err, connection) {
		connection.query('INSERT INTO driver (name, surname, patronymic, birthdate, open_category, license, photo, phone_number, comment, status, bus_binding, company_id) values ("'+req.body.name+'", "'+req.body.surname+'", "'+req.body.patronymic+'", "'+req.body.birthdate+'", "'+req.body.open_category+'", "'+req.body.license+'", "'+/*req.files.photo.originalFilename*/req.file.filename+'", "'+req.body.phone_number+'", "'+req.body.comment+'", "'+req.body.status+'", "'+req.body.bus_binding+'", "'+req.body.company_id+'")', function (err, results, fields) {
